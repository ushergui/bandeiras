...
def gerar_audios_continentes():
    ...
    output_folder_paises = "assets/audio/continente_do_pais"
    output_folder_continentes = "assets/audio/pais_do_continente"
    voice_id = "cgSgspJ2msm6clMCkdW9" # Voz "Jessica"

    try:
        client = ElevenLabs(api_key=CHAVE_API)
        os.makedirs(output_folder_paises, exist_ok=True)
        os.makedirs(output_folder_continentes, exist_ok=True)
        
        print(f"--- MODO CONTINENTES ---")

        # Modo 1: "Qual o continente do [País]?"
        print("\nGerando áudios: Qual o continente do país?")
        for pais in paises_info:
            nome_pais, artigo_pais = pais['nome'], pais['artigo']
            texto_pergunta = f'Qual é o continente {artigo_pais} {nome_pais}?'
            nome_arquivo = f"{nome_pais.lower().replace(' ', '_')}.mp3"
            caminho_completo = os.path.join(output_folder_paises, nome_arquivo)
            
            if os.path.exists(caminho_completo): continue
            ...
            print(f" -> Áudio para '{nome_pais}' gerado.")

        # Modo 2: "Qual país pertence ao [Continente]?"
        print("\nGerando áudios: Qual país pertence ao continente?")
        continentes = sorted(list(set(p['continente'] for p in paises_info))) # Pega continentes únicos
        artigos_continentes = {
            'América do Norte': 'da', 
            'América Central': 'da', 
            'América do Sul': 'da', 
        ...
        for continente in continentes:
            artigo = artigos_continentes.get(continente, 'do')
            texto_pergunta = f'Qual destes países faz parte {artigo} {continente}?'
            nome_arquivo = f"{continente.lower().replace(' ', '_')}.mp3"
            caminho_completo = os.path.join(output_folder_continentes, nome_arquivo)

            if os.path.exists(caminho_completo): continue

            audio_gerado = client.text_to_speech.convert(voice_id=voice_id, text=texto_pergunta, model_id="eleven_multilingual_v2")
            save(audio_gerado, caminho_completo)
            print(f" -> Áudio para '{continente}' gerado.")

        print("\n✅ Áudios de continentes gerados com sucesso!")
    except Exception as e:
        print(f"\nOcorreu um erro inesperado: {e}")

if __name__ == "__main__":
    gerar_audios_continentes()
